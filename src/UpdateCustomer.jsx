...
class UpdateCustomer extends Component {
    ...
    componentDidMount = async () => {
        ...
        console.log(id);
        var response = await fetch(`http://localhost:5000/customers/${id}`, { method: "GET" });
        var body = await response.json();
        this.setState(
            {
                name: body.name, city: body.city, phone: body.phone, photo: body.photo
            });
    };
    onSave = async (event) => {
        ...
        var response = await fetch(`http://localhost:5000/customers/${id}`, { method: "PUT", body: JSON.stringify(customer), headers: { "content-type": "application/json" } });
        var body = await response.json();
        console.log(body);
        if (body) {
